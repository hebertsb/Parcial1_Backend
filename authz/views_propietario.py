    ...
    def post(self, request):
        ...
        try:
            ...
            try:
                ...
                nombre_archivo = f"propietario_{usuario.id}_{timestamp}.{extension}"
                
                # Usar carpeta específica para fotos de propietarios
                folder_path = f"/PropietariosReconocimiento/propietario_{usuario.id}"
                resultado_upload = upload_image_to_dropbox(foto, nombre_archivo, folder_path)
                
                # Verificar que la subida fue exitosa
                if not resultado_upload or not resultado_upload.get('url'):
                    return Response({
                        'success': False,
                        'error': 'Error al obtener URL de la foto subida'
                    }, status=500)
