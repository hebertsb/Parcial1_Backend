...
#     tipo = serializers.ChoiceField(choices=(('expensa', 'expensa'), ('multa', 'multa'), ('reserva', 'reserva')))
...
class RegistrarPagoSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        # Lógica de validación para cada tipo de pago
        if tipo == 'expensa':
            expensa = ExpensasMensuales.objects.filter(id=objetivo_id, vivienda__persona=persona).first()
            if not expensa:
                raise serializers.ValidationError('La expensa indicada no existe o no pertenece al usuario.')
            pendiente = max(Decimal('0'), expensa.monto_total - total_pagado_expensa(expensa))
            if pendiente <= 0:
            ...
            multa = MultasSanciones.objects.filter(id=objetivo_id, persona_infractor=persona).first()
            if not multa:
                multa = MultasSanciones.objects.filter(id=objetivo_id, persona_responsable=persona).first()
            if not multa:
                raise serializers.ValidationError('La multa indicada no existe o no pertenece al usuario.')
            pendiente = max(Decimal('0'), multa.monto - total_pagado_multa(multa))
            if pendiente <= 0:
            ...
            reserva = ReservaEspacio.objects.filter(id=objetivo_id).first()
            if not reserva:
    ...
    def create(self, validated_data):
        ...
        usuario = self.context['request'].user
        expensa = validated_data.get('expensa')
        multa = validated_data.get('multa')
        ...
        with transaction.atomic():
            ...
            pago = Pagos.objects.create(
                persona=persona,
                tipo_pago='expensa' if expensa else 'multa' if multa else 'reserva',
                expensa=expensa,
                multa=multa,
                reserva=reserva,  # Añadir la reserva en el pago
                monto=monto,
                metodo_pago=metodo_pago,
                numero_comprobante=referencia_generada,
                estado='procesado',
            ...
            # Actualizar el estado de la expensa
            if expensa:
                total = total_pagado_expensa(expensa)
                expensa.estado = 'pagada' if total >= expensa.monto_total else 'parcial'
                expensa.save(update_fields=['estado'])
            ...
            if reserva:
                total = total_pagado_reserva(reserva)
                reserva.estado = 'pagada' if total >= reserva.monto_total else 'parcial'
                reserva.fecha_pago = timezone.now()
                reserva.save(update_fields=['estado', 'fecha_pago'])
